...
interface OrderRepo extends PagingAndSortingRepository<Order, String> {
	...
	List<Order> findByStateAndBelongDept(String state, Dept dept, Pageable pageable);
	...
	@Query("select o from LongOrder o where o.visit = :visit and o.state = :state")
	List<LongOrder> findLongOrder(@Param("visit") Visit visit, @Param("state") String state,
			Pageable pageable);
